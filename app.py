...
with st.sidebar:
    ...
    st.divider()
    st.header("👥 成員管理")
    new_friend = st.text_input("新增朋友到名單")
    if st.button("確認新增"):
    ...
    if mode == "🍽️ 聚餐支出":
        ...
        if st.button("儲存聚餐紀錄"):
            if total_amount <= 0 or not attendees or not payers:
                ...
                new_row = [str(date), ",".join(payers), total_amount, f"聚餐: {','.join(attendees)}"]
                for h in headers[4:]:
                    friend = h.strip()
...
with tab2:
    ...
    if len(all_data) <= 1:
        ...
        if st.button("🔥 本輪結帳完畢，開始新紀錄"):
            # 1. 產生歸檔名稱 (例如 2024-04-15-1430-歸檔)
